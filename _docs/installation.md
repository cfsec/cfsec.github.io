...
1. Copy the public key block into a new file, eg; /tmp/cfsec_key.txt
   
2. Import the gpg key
   ```cmd
    gpg --import < /tmp/cfsec_key.txt
   ```
3. Verify that the key has been imported
    ```cmd
    gpg --list-keys signing@cfsec.dev
    ```
    check the output is
    ```
    pub  rsa4096 2021-05-20 [SC]       D66B222A3EA4C25D5D1A097FC34ACEFB46EC39CE
    uid  [ultimate] cfsec Signing (Code signing for cfsec) <signing@cfsec.dev>
    sub  rsa4096 2021-05-20 [E]
    ```
4. Download the required binary and its associated sig file
   
5. Assuming it is `cfsec-linux-amd64` the verify command below should be run, passing in the sig file then the binary file
   ```cmd
   gpg --verify cfsec-linux-amd64.D66B222A3EA4C25D5D1A097FC34ACEFB46EC39CE.sig cfsec-linux-amd64
   ```
